...
Mixture GAN (MGAN) \cite{hoang2018mgan} is similar to MAD-GAN except that the classifer and the discriminator are separate. The discriminator is two-class as usual discriminating between true and fake examples, and there is a separate $K$-class classifier only for the fake examples.
...
\label{sec:fmgan}
Flat mixture of generators model, which we call ME-GAN (although the name is similar to MEGAN, we fully use the ME model unlike MEGAN, therefore keep the name with a dash), consists of local generators $\{g_i(z)\}_{i=1}^{K}$, a gating function $\alpha(z)$. The idea is that instead of learning a global generator, we divide the input space into regions and learn a set of local generators.
\begin{gather}
...
\label{sec:hmgan}
Just like the hierarchical mixture of experts \cite{jordan1994hierarchical} go from the flat organization of mixture of experts  \cite{jacobs1991adaptive} to a tree, our proposed hierarchical mixture of generators (HME-GAN) go from a flat mixture of generators to a tree. HME-GAN and ME-GAN are the same except the formulation of the gating function. Let us think of a binary decision tree. Generators are located on the leaves of this tree. At each internal node $m$ of the tree, there is a logistic function $\alpha_m$ with parameters $\{w_{\alpha}^{(m)}, b_{\alpha}^{(m)}\}$:
\begin{equation}
...
\label{sec:toy}
For a visual understanding of the model, let us look at Figure \ref{fig:toy}. We are given a spiral-like shaped two-dimensional data set. We trained an HME-GAN model with a depth of 3, therefore 8 linear generators. In Figure \ref{fig:toy:samples}, we see the trained model's response. To see which generator is responsible for which part, we pick a color for each generator and visualize its response with the respective color. The responsibility of each generator is calculated by softly counting the gating function's output for each point. This soft count decides the transparency level (alpha level) of the visualized point. This is shown in Figure \ref{fig:toy:coop}. The horizontal line in Figure \ref{fig:toy:coop} shows the color of each generator. As we expect, we see that the color changes when we change the region. Responses of each generator alone is also shown in Figure \ref{fig:toy:experts}. We understand that some generators are not used at all.

\begin{figure}[htbp]
\begin{center}
\subfloat[Blue dots are real samples, red dots are generated samples.]{
	\includegraphics[width=0.45\linewidth]{hme_out.pdf}
	\label{fig:toy:samples}
}
\subfloat[Cooperation of experts]{
	\includegraphics[width=0.45\linewidth]{hme_coop.pdf}
	\label{fig:toy:coop}
}

\subfloat[Local experts]{
	\includegraphics[width=0.98\linewidth]{hme_experts.pdf}
	\label{fig:toy:experts}
}
\end{center}
\caption{Results of training a HME-GAN on a spiral data set.}
\vskip\baselineskip
\label{fig:toy}
\end{figure}
%

\section{The Convolutional Pipeline}
\label{sec:conv}

Our experiments are done in the image domain. It is known that using a convolutional architecture for tasks that involve images increases performance dramatically. For example, although the set of human face images contain many modes, textures are quite the same. For this reason, we incorporate transposed convolutional (also known as deconvolutional) layers in both of our models. Instead of generating samples directly in the data domain $x$, our mixture models first generates an intermediate hidden representation $h$, that is given to a transposed convolutional architecture $G_s$. This architecture then produces the output $x$. For the human face example, local generators create an intermediate representation which may hold information about the location and the color of eyes,  and $G_s$ produces eyes given these features. For other domains where data points share common features, another domain specific architecture can be used to increase the performance given that the architecture is differentiable. The pipelines for ME-GAN and HME-GAN are depicted in Figure \ref{fig:models:me-gan} and \ref{fig:models:hmegan} respectively. Note that it is only $G$ that is modeled with ME or HME, $D$ remains the usual deep convolutional neural network.
...
\begin{center}
\subfloat[MNIST]{
	\includegraphics[width=0.45\linewidth]{mnist_dataset.png}
	\label{fig:dataset:mnist}
}
\subfloat[FashionMNIST]{
	\includegraphics[width=0.45\linewidth]{fashion_dataset.png}
	\label{fig:dataset:fashion}
}

\subfloat[CelebA]{
	\includegraphics[width=0.45\linewidth]{celeba_dataset.png}
	\label{fig:dataset:celeba}
}
\subfloat[UTZap50K]{
	\includegraphics[width=0.45\linewidth]{utzap50k_dataset.png}
	\label{fig:dataset:utzap50k}
}

\subfloat[Oxford Flowers]{
	\includegraphics[width=0.45\linewidth]{flowers_dataset.png}
...
These settings are applied to all experiments unless otherwise stated. The convolutional architecture that we use is the deep convolutional GAN (DCGAN) \cite{radford2015unsupervised}. Since MNIST and FashionMNIST are $32\times 32$ pixels and other data sets are $64\times 64$ pixels, we used different networks with different sizes. The network architectures used for the former and latter data sets are given in Table \ref{tab:models} that shows the number of units in each layer and the convolutional structure. Layers 2 to 5 are transposed convolutional layers and layer 1 is fully connected for FC, or is where we have the HME or ME structure embedded. The dimensionality of the input $z$ is set to 100. We did not use normalization in $G$ but used layer normalization \cite{lei2016layer} in $D$.
...
\subfloat[ME-64 leafs]{
	\includegraphics[width=0.45\linewidth]{me_leafs_fashion.png}
	\label{fig:leafs:me_fashion}
}
\subfloat[HME-6 leafs]{
	\includegraphics[width=0.45\linewidth]{hme_leafs_fashion.png}
...
\begin{center}
\subfloat{
	\includegraphics[width=0.45\linewidth]{me_samples_mnist.png}
	\label{fig:samples:me_mnist}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_samples_mnist.png}
	\label{fig:samples:hme_mnist}
}

\subfloat{
	\includegraphics[width=0.45\linewidth]{me_samples_fashion.png}
	\label{fig:samples:me_fashion}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_samples_fashion.png}
	\label{fig:samples:hme_fashion}
}

\subfloat{
	\includegraphics[width=0.45\linewidth]{me_samples_celeb.png}
	\label{fig:samples:me_celeb}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_samples_celeb.png}
	\label{fig:samples:hme_celeb}
}

\subfloat{
	\includegraphics[width=0.45\linewidth]{me_samples_utzap50k.png}
	\label{fig:samples:me_utzap50k}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_samples_utzap50k.png}
	\label{fig:samples:hme_utzap50k}
}

\subfloat{
	\includegraphics[width=0.45\linewidth]{me_samples_flowers.png}
	\label{fig:samples:me_flowers}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_samples_flowers.png}
...
\begin{center}
\subfloat[ME-64]{
	\includegraphics[width=0.45\linewidth]{cov_me.pdf}
	\label{fig:cov:me}
}
\subfloat[HME-6]{
	\includegraphics[width=0.45\linewidth]{cov_hme.pdf}
	\label{fig:cov:hme}
}
\end{center}
...
\subfloat[ME-L-16 leafs]{
	\includegraphics[width=0.45\linewidth]{me_lin_leafs_fashion.png}
	\label{fig:leafs_lin:me_fashion}
}
\subfloat[HME-L-4 leafs]{
	\includegraphics[width=0.45\linewidth]{hme_lin_leafs_fashion.png}
...
\begin{center}
\subfloat{
	\includegraphics[width=0.45\linewidth]{me_lin_samples_mnist.png}
	\label{fig:samples_lin:me_mnist}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_lin_samples_mnist.png}
	\label{fig:samples_lin:hme_mnist}
}

\subfloat{
	\includegraphics[width=0.45\linewidth]{me_lin_samples_fashion.png}
	\label{fig:samples_lin:me_fashion}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_lin_samples_fashion.png}
	\label{fig:samples_lin:hme_fashion}
}

\subfloat{
	\includegraphics[width=0.45\linewidth]{me_lin_samples_celeb.png}
	\label{fig:samples_lin:me_celeb}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_lin_samples_celeb.png}
	\label{fig:samples_lin:hme_celeb}
}

\subfloat{
	\includegraphics[width=0.45\linewidth]{me_lin_samples_utzap50k.png}
	\label{fig:samples_lin:me_utzap50k}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_lin_samples_utzap50k.png}
	\label{fig:samples_lin:hme_utzap50k}
}

\subfloat{
	\includegraphics[width=0.45\linewidth]{me_lin_samples_flowers.png}
	\label{fig:samples_lin:me_flowers}
}
\subfloat{
	\includegraphics[width=0.45\linewidth]{hme_lin_samples_flowers.png}
...
\begin{center}
\subfloat[ME-64]{
	\includegraphics[width=0.45\linewidth]{cov_me_lin.pdf}
	\label{fig:cov_lin:me}
}
\subfloat[HME-6]{
	\includegraphics[width=0.45\linewidth]{cov_hme_lin.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.9\columnwidth]{polar_mnist.pdf}
\end{center}
\caption{MNIST.}
\vskip\baselineskip
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.9\columnwidth]{polar_fashion.pdf}
\end{center}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.9\columnwidth]{polar_celeb.pdf}
\end{center}
\caption{CelebA.}
\vskip\baselineskip
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.9\columnwidth]{polar_utzap50k.pdf}
\end{center}
\caption{UTZap50K.}
\vskip\baselineskip
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.9\columnwidth]{polar_flowers.pdf}
\end{center}
\caption{Oxford Flowers.}
\vskip\baselineskip
