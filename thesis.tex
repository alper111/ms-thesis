...
\end{center}
\caption[A spiral data set]{A spiral data set. The data is one-dimensional, if we know $x_1=r*cos(r)$, $x_2=r*sin(r)$.}
\vskip\baselineskip
\label{fig:spiral}
\end{figure}
...
\end{center}
\caption[GAN framework]{GAN framework. Given an input $z$ that is drawn from $p(z)$, the generator $G$ produces a \emph{fake} sample, $x_{fake}$. A \emph{real} sample $x_{real}$ that is drawn from the data set $X$ is given to the discriminator $D$ together with $x_{fake}$. $D$ is trained to classify $x_{fake}$ as 0 and $x_{real}$ as 1.}
\vskip\baselineskip
\label{fig:gan}
\end{figure}
%
...
\end{center}
\caption[An example of a mode collapse]{An example of a mode collapse. The data set consists of samples from two Gaussian distributions, one on the top left with low variance and the other on the bottom right with high variance. The generator model fails to model the one on the top left.}
\vskip\baselineskip
...
\begin{table}[thbp]
\vskip\baselineskip
\caption[The generator and the discriminator architectures for $64 \times 64$ sized data sets]{The generator and the discriminator network architectures that are used with CelebA , UTZap50K and Oxford Flowers which are $64\times 64$ images in color. For convolutional layers, kernel size is set to $4 \times 4$ with a stride of $2 \times 2$ and with a padding of 1.}
\begin{center}
\begin{tabular}{|l|c|c|c|c|}
\hline
\multicolumn{5}{|c|}{Generator Network}\\
...
\end{center}
\caption[The average responses of tree nodes of HME-GAN on a spiral data set]{The average responses of tree nodes of HME-GAN on a spiral data set. The data set is represented with black cross. The output of the model is colored dots. These colors represent different regions of the input. As we go down in the tree, each node becomes more specialized.}
\vskip\baselineskip
\label{fig:toy}
\end{figure}
...
\end{center}
\caption[Average responses of generators at the leaves for ME-64 and HME-6]{Average responses of generators at the leaves for ME-64 and HME-6. We see that HME-6 responses are more localized compared to ME-64.}
\vskip\baselineskip
\label{fig:leaves}
\end{figure}
%
...
\begin{table}[thbp]
\vskip\baselineskip
\caption[5-NN accuracies and FID scores of ME and HME models on UTZap 50K data set]{5-NN accuracies and FID scores of ME and HME models on UTZap50K data set}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\end{center}
\caption[Covariance matrix of gating values of generators for ME-L-16 and HME-L-4]{Covariance matrix of gating values of generators for ME-L-16 and HME-L-4. As in Figure \ref{fig:covariances} (constant model), we see spectral squares near diagonal for HME-L-4 model. The diagonal of the covariance matrix for ME-L-16 is more intense compared to its constant counterpart in Figure \ref{fig:cov:me}.}
\vskip\baselineskip
...
\end{center}
\caption[Some samples generated from generators at the leaves of ME-L-16 and HME-L-4 for MNIST data set]{Some samples generated from generators at the leaves of ME-L-16 and HME-L-4 for MNIST data set. Samples in the $i$th column are generated from $i$th generator. When we look at columns from 5 to 8 in (b), they are all digits that contain a vertical stroke. Since HME-L-4 is a tree with a depth of four, generators from 5 to 8 have the same ancestor at the second level of the tree. We see that digits that contain a vertical stroke are split from others at the second level of the tree. These digits are further split into different leaves when we go down at the bottom of the tree. This further confirms our hypothesis that HME hierarchically splits the data generation task.}
\vskip\baselineskip
\label{fig:gens}
\end{figure}
