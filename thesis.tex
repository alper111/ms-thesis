...
\end{center}
\caption[Mixture of experts GAN]{Mixture of experts GAN \cite{park2018megan}. Given $z$, each generator $G_i$ outputs a sample $x_i$. The gating network selects one generated sample among all based on $z$ and feature maps of generated samples. Here, $\phi$ returns the activation map of $x_i$ from earlier layers of $G_i$. Unlike the original mixtures of experts \cite{jacobs1991adaptive}, the selection is hard: $(a_1, a_2, a_3)$ is a one-hot vector.}
\vskip\baselineskip
...
\end{center}
\caption[ME-GAN model.]{ME-GAN model. Given $z$, each generator outputs a latent representation. These representations are mixed with weights $a_i$ which are provided through the gating function. The gating function consists of a linear layer with a softmax function. The shared network $G_s$ produces the final output.}
\vskip\baselineskip
...
\end{center}
\caption[HME-GAN model.]{HME-GAN model. Each generator creates a latent representation from $z$ as in Figure \ref{fig:models:me-gan}. However the calculation of the gating values is different. Each $a_i$ is calculated by multiplying the $\sigma$ values along the path from the leaf to the root (Equation \ref{eq:hm_mix}). The value on a path is $\sigma$ if the path is a left child, $1-\sigma$ otherwise. As in ME-GAN, $a_i$ values sum up to one.}
\vskip\baselineskip
...
In Figure \ref{fig:gens} and \ref{fig:gens2}, we show some samples generated from different generators at the leaves of ME-L-16 and HME-L-4. Each generator works in different input region. To find these input regions, we samples 10,000 input noise vectors and select the top five most likely points. Here, the most likely point for a generator is the point which maximizes the gating value of a generator. From the figure, we see that each generator is localized in a different input-output mapping region.
...
\end{center}
\caption{Covariance matrix of gating values of generators for ME-L-16 and HME-L-4. As in Figure \ref{fig:covariances} (constant model), we see spectral squares near diagonal for HME-L-4 model. The diagonal of the covariance matrix for ME-L-16 is more intense compared to its constant counterpart in Figure \ref{fig:cov:me}.}
\vskip\baselineskip
...
\begin{center}
\subfloat[ME-L-16]{
	\includegraphics[width=0.98\linewidth]{me_mnist_L4_exps.png}
	\label{fig:gens:mnist:me}
}

\subfloat[HME-L-4]{
	\includegraphics[width=0.98\linewidth]{hme_mnist_L4_exps.png}
	\label{fig:gens:mnist:hme}
}
\end{center}
\caption{Some samples generated from generators at the leaves of ME-L-16 and HME-L-4 for MNIST data set. Samples in the $i$th column are generated from $i$th generator. When we look at columns from 5 to 8 in (b), they are all digits that contain a vertical stroke. Since HME-L-4 is a tree with a depth of four, generators from 5 to 8 have the same ancestor at the second level of the tree. We see that digits that contain a vertical stroke are split from others at the second level of the tree. These digits are further split into different leaves when we go down at the bottom of the tree. This further confirms our hypothesis that HME hierarchically splits the data generation task.}
\vskip\baselineskip
\label{fig:gens}
\end{figure}
...
\begin{center}
\subfloat[ME-L-16]{
	\includegraphics[width=0.98\linewidth]{me_celeb_L4_exps.png}
	\label{fig:gens:celeb:me}
}

\subfloat[HME-L-4]{
	\includegraphics[width=0.98\linewidth]{hme_celeb_L4_exps.png}
	\label{fig:gens:celeb:hme}
}
\end{center}
\caption{Some samples generated from generators at the leaves of ME-L-16 and HME-L-4 for CelebA data set. Samples in the $i$th column are generated from $i$th generator. Again, for HME model we see that generators that are neighbors create images that share common features such as background color, hair type, gender. For example, male faces are generally located in first eight columns which indicates that the tree is softly splitted by the gender feature at the first level. This is actually a good split since the split by the gender approximately splits $p(x)$ by half. When we go down in the tree, splits become more detailed. In columns 11 and 12, we see women with different hair colors but with the same orientation and hair type.}
\vskip\baselineskip
\label{fig:gens2}
\end{figure}
