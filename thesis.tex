...
\label{eq:kde}
\end{equation}
where $K$ is the kernel function, and $h$ is called \emph{bandwidth}. $K$ and $h$ changes the function family of the model. These are not parameters of the model as in parametric methods. For example, the degree of the polynomial in parametric methods can be analogous to $K$ and $h$, hyperparameters of the model. $K$ can be any function that satisfies $\int K(u) du = 1$. A simple example, also known as the rectangular kernel:
\begin{equation}
