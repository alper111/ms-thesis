...
Another important point is the way  the input is represented. For example in Figure \ref{fig:spiral}, if we use Cartesian coordinates, we probably fail, or find poor estimates for $p(x)$. If we know the transformation to polar coordinates, then we get a simpler problem. Both parametric models and non-parametric models will likely to yield better results in polar coordinates for this specific problem. Therefore, the transformation is the golden nugget.
...
\end{center}
\caption{GAN framework. Given an input $z$ that is drawn from $p(z)$, the generator $G$ produces a \emph{fake} sample, $x_{fake}$. A \emph{real} sample $x_{real}$ that is drawn from the data set $X$ is given to the discriminator $D$ together with $x_{fake}$. $D$ is trained to classify $x_{fake}$ as 0 and $x_{real}$ as 1.}
\vskip\baselineskip
\label{fig:gan}
\end{figure}
%
...
\end{center}
\caption{An example of a mode collapse. The data set consists of samples from two Gaussian distributions, one on the top left with low variance and the other on the bottom right with high variance. The generator model fails to model the one on the top left.}
\vskip\baselineskip
...
\end{center}
\caption[Multi-agent diverse GAN]{Multi-agent diverse GAN \cite{ghosh2018multi}. Given an input noise vector $z$, the shared network produces an intermediate representation $z'$. From this representation, each generator $G_i$ outputs a sample. The discriminator makes a $K+1$-class classification, one class for the real sample, and the other $K$ classes for the index of the generator.}
\vskip\baselineskip
\label{fig:models:madgan}
\end{figure}

The model is shown in Figure \ref{fig:models:madgan}. Given $z$, a shared neural network block produces $z'$, an intermediate representation that is higher dimensional than $z$, which is used by a set of generators $\{G_i(z')\}_{i=1}^K$. The discriminator is a $K+1$-class classifier with 0 for true, and $1$ to $K$ as the index of the generator for the fake instances. The discriminator should push the different generators to different modes to be able to solve the classification problem. More formally, the discriminator tries to minimize the following:
\begin{equation}
\min_{\phi} \quad -\mathbb{E}_{x \sim (p_t \cup p_f)} \left[ \sum_{j=0}^K r_j(x) \log D_j(x;\phi)\right]
\label{eq:madgan1}
\end{equation}
where $p_t$  and $p_f$ are the target and the fake distribution respectively, and $r$ is a one-hot vector with $K+1$ length. This is the regular cross-entropy error function. The cost function for generators:
\begin{equation}
\min_{\theta} \quad \mathbb{E}_{z \sim p(z)} \log (1-D_0(G_i(z;\theta)))
\label{eq:madgan2}
\end{equation}
where $G_i$ is the $i$th generator, and $D_0$ represents the discriminator's probability output for real class.

Though there are multiple generators, we do not mix them in a cooperative manner. We also do not partition $p(z)$ and use each partition for different generators. This should rather be thought as each generator produces their own interpretation of $p(z)$. Therefore, instead of partitioning $p(z)$ we introduce alternative generator functions for the same region in $p(z)$.
%
%
\section{Mixture GAN}
\label{sec:mgan}
...
\end{center}
\caption[Mixture GAN]{Mixture GAN \cite{hoang2018mgan}. This is similar to MAD-GAN architecture except they use multiple generators at the earlier layers. Each generator $G_i$ creates an abstract representation which is fed to the shared network to generate the output. One sample out of $K$ generated samples is selected at random and given to the discriminator. The discriminator network has two heads at the last layer. One head predicts whether the sample is real or fake, and the other predicts the index of the generator.}
\vskip\baselineskip
\label{fig:models:mgan}
\end{figure}

The model is shown in Figure \ref{fig:models:mgan}. There is also the difference that the split of the generators is earlier. A set of generators $\{G_i(z)\}_{i=1}^K$, transform $z$ and for all, the shared network $G_s$ produces the final output. A multinomial distribution is sampled to randomly select one of the generators. The parameters of the multinomial distribution are fixed. While the discriminator tries to discriminate between the fake and the real data as usual, the classifier tries to predict the index of the generator that produced the fake sample. These two networks share parameters treating the training of the discriminator/classifier as a multi-task learning problem.
...
\end{center}
\caption[Mixture of experts GAN]{Mixture of experts GAN \cite{park2018megan}. Given an input vector $z$, each generator $G_i$ outputs a sample.}
\vskip\baselineskip
...
\begin{center}
\subfloat[MNIST]{
	\includegraphics[width=0.9\linewidth]{mnist_dataset.png}
	\label{fig:dataset:mnist}
}

\subfloat[FashionMNIST]{
	\includegraphics[width=0.9\linewidth]{fashion_dataset.png}
	\label{fig:dataset:fashion}
}

\subfloat[CelebA]{
	\includegraphics[width=0.9\linewidth]{celeba_dataset.png}
	\label{fig:dataset:celeba}
}

\subfloat[UTZap50K]{
	\includegraphics[width=0.9\linewidth]{utzap50k_dataset.png}
	\label{fig:dataset:utzap50k}
}

\subfloat[Oxford Flowers]{
	\includegraphics[width=0.9\linewidth]{flowers_dataset.png}
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{The generator and the discriminator network architectures that are used with CelebA , UTZap50K and Oxford Flowers which are $64\times 64$ images in color. For convolutional layers, kernel size is set to $4 \times 4$ with a stride of $2 \times 2$ and with a padding of 1.}
\begin{center}
\begin{tabular}{|l|c|c|c|c|}
\hline
\multicolumn{5}{|c|}{Generator Network}\\
...
This example shows that even when the distribution is very non-linear, we can model it with simple linear models by combining them in a hierarchical manner. The hierarchical combination dissects the problem into two at each level, easing the problem in a divide-and-conquer fashion.

\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.9\textwidth]{toy.pdf}
\end{center}
\caption{The average responses of tree nodes of HME-GAN on a spiral data set. The data set is represented with black cross. The output of the model is colored dots. These colors represent different regions of the input. As we go down in the tree, each node becomes more specialized.}
\vskip\baselineskip
\label{fig:toy}
\end{figure}

\section{Flat Mixture vs. Hierarchical Mixture}
We have shown two different ways of combining multiple generators, namely, the flat mixture and the hierarchical mixture. Now, we want to see whether there is a qualitative difference between these two models. We use trees of different depths, and we also test a flat mixture of generators with equal number of leaves. For example, we have a tree of depth five with 32 leaves and a flat mixture of 32 leaves. In the former case, for each leaf, we have five binary gatings; in the latter case, there is one gating that chooses one of 32. For the hierarchical model, we tested trees with depth 5, 6, 7 and 8. To get the same number of leaves, we used 32, 64, 128 and 256 generator experts in the flat mixture.

\begin{figure}[htbp]
\begin{center}
\subfloat{
	\includegraphics[width=0.6\linewidth]{me_samples_mnist.png}
	\label{fig:samples:me_mnist}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{me_samples_fashion.png}
	\label{fig:samples:me_fashion}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{me_samples_celeb.png}
	\label{fig:samples:me_celeb}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{me_samples_utzap50k.png}
	\label{fig:samples:me_utzap50k}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{me_samples_flowers.png}
	\label{fig:samples:me_flowers}
}
\end{center}
\caption{Samples generated using ME-64.}
\vskip\baselineskip
...
\begin{center}
\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_samples_mnist.png}
	\label{fig:samples:hme_mnist}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_samples_fashion.png}
	\label{fig:samples:hme_fashion}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_samples_celeb.png}
	\label{fig:samples:hme_celeb}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_samples_utzap50k.png}
	\label{fig:samples:hme_utzap50k}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_samples_flowers.png}
	\label{fig:samples:hme_flowers}
}
\end{center}
\caption{Samples generated using HME-6.}
\vskip\baselineskip
...
\begin{center}
\subfloat[ME-64]{
	\includegraphics[width=0.6\linewidth]{me_leaves_fashion.png}
	\label{fig:leaves:me_fashion}
}

\subfloat[HME-6]{
	\includegraphics[width=0.6\linewidth]{hme_leaves_fashion.png}
	\label{fig:leaves:hme_fashion}
}
\end{center}
\caption{Average responses of generators at the leaves for ME-64 and HME-6. We see that HME-6 responses are more localized compared to ME-64.}
\vskip\baselineskip
\label{fig:leaves}
\end{figure}
%
...
\end{center}
\caption{Covariance matrix of gating values of generators for ME-64 and HME-6. We see spectral squares near diagonal for HME-6 model, which indicates generators that are closer by index are used together.}
\vskip\baselineskip
...
\subfloat[MNIST]{
	...
	\label{fig:box_s:mnist}
}

\subfloat[FashionMNIST]{
	\includegraphics[width=0.95\linewidth]{box_fashion_s.pdf}
	\label{fig:box_s:fashion}
}
\end{center}
\caption{FID scores and 5-NN accuracies of ME-$k$ and HME-$k$ on MNIST and FashionMNIST data sets. Lower FID and 5-NN scores are better. The parameter count does not include the convolutional part.}
\vskip\baselineskip
\label{fig:box_s1}
\end{figure}
...
\subfloat[CelebA]{
	...
	\label{fig:box_s:celeb}
}

\subfloat[UTZap50K]{
	\includegraphics[width=0.95\linewidth]{box_utzap50k_s.pdf}
	\label{fig:box_s:utzap50k}
}

\subfloat[Oxford Flowers]{
	\includegraphics[width=0.95\linewidth]{box_flowers_s.pdf}
	\label{fig:box_s:flowers}
}
\end{center}
\caption{FID scores and 5-NN accuracies of ME-$k$ and HME-$k$ on CelebA, UTZap50K, and Oxford Flowers data sets. Lower FID and 5-NN scores are better. The parameter count does not include the convolutional part.}
\vskip\baselineskip
\label{fig:box_s2}
\end{figure}
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores of ME and HME models on MNIST data set.}
\begin{center}
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores of ME and HME models on FashionMNIST data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores of ME and HME models on CelebA data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores of ME and HME models on UTZap50K data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores of ME and HME models on Oxford Flowers data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
When we make a comparison with the baseline, we see that ME and HME are not as good as FC in terms of FID score or 5-NN accuracy (Figure \ref{fig:box_s1} and \ref{fig:box_s2}). Their performance improves as the structure gets larger; note that trees with depth 5 and 6 are smaller than FC. Although 5-NN real accuracies are quite close, there is a gap between 5-NN fake accuracies. High 5-NN fake accuracy implies that fake samples are located near fake samples in $p(x)$. So, the performance drop might be due to decreased sample diversity.

One possible cause for the decreased diversity is to use constant vectors in the leaves. In a FC layer, the random vector $z$ is gone through an affine transformation and a ReLU non-linearity, the \emph{randomness} is propagated linearly. ME and HME, on the other hand, encapsulate the information of $z$ (randomness) in their gating values. Gating values are calculated with a set of sigmoid functions for HME, and with a softmax function for ME. Both the sigmoid function and the softmax function get saturated for values that are too low or too high. Therefore, if gating weights get too high or too low, which also means that it mimics a hard split instead of a soft one, the variety is lost.

To remedy this problem, we use linear functions as in Equation \ref{eq:linear} at leaves of both ME and HME models. We experimented with number of generators set to 4, 8, 16 and 32. We call these models as ME-L-$k$ and HME-L-$k$ where $k$ is the number of generators for the flat mixture and the depth level for the hierarchical mixture.

\begin{figure}[htbp]
\begin{center}
\subfloat{
	\includegraphics[width=0.6\linewidth]{me_lin_samples_mnist.png}
	\label{fig:samples_lin:me_mnist}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{me_lin_samples_fashion.png}
	\label{fig:samples_lin:me_fashion}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{me_lin_samples_celeb.png}
	\label{fig:samples_lin:me_celeb}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{me_lin_samples_utzap50k.png}
	\label{fig:samples_lin:me_utzap50k}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{me_lin_samples_flowers.png}
	\label{fig:samples_lin:me_flowers}
}
\end{center}
\caption{Samples generated using ME-L-16.}
\vskip\baselineskip
...
\begin{center}
\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_lin_samples_mnist.png}
	\label{fig:samples_lin:hme_mnist}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_lin_samples_fashion.png}
	\label{fig:samples_lin:hme_fashion}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_lin_samples_celeb.png}
	\label{fig:samples_lin:hme_celeb}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_lin_samples_utzap50k.png}
	\label{fig:samples_lin:hme_utzap50k}
}

\subfloat{
	\includegraphics[width=0.6\linewidth]{hme_lin_samples_flowers.png}
	\label{fig:samples_lin:hme_flowers}
}
\end{center}
\caption{Samples generated using HME-L-4.}
\vskip\baselineskip
...
\subfloat[ME-L-16]{
	...
	\label{fig:leaves_lin:me_fashion}
}

\subfloat[HME-L-4]{
	\includegraphics[width=0.9\linewidth]{hme_lin_leaves_fashion.png}
	\label{fig:leaves_lin:hme_fashion}
}
\end{center}
\caption{Average responses of generators at the leaves for ME-L-16 and HME-L-4.}
\vskip\baselineskip
...
When we compare ME-L and HME-L models, we see that ME slightly enjoys more from having linear experts. We visualized the average generator responses for FashionMNIST data set in Figure \ref{fig:leaves_lin}. We see that ME-L leaves are now more local and diverse. Figure \ref{fig:cov_lin:me} also confirms this since the diagonal is now more prominent. On the other hand, in Figure \ref{fig:cov_lin:hme} HME seems to use its leaves in a more cooperative way when compared with its constant counterpart. For both formulations, results stagnates at their largest models. We argue that the training time may increase when the number of generators increase since we distribute the error to multiple generators. More generators imply we update generators with smaller gradients. Also, the bottleneck may be caused by other factors.

In Figure \ref{fig:gens}, we show some samples generated from different generators at the leaves of ME-L-16 and HME-L-4. Each generator works in different input region. To find these input regions, we samples 10,000 input noise vectors and select the top five most likely points. Here, the most likely point for a generator is the point which maximizes the gating value of a generator. From the figure, we see that each generator is localized in a different input region.

\begin{figure}[htbp]
\begin{center}
\subfloat[ME-L-16]{
	\includegraphics[width=0.6\linewidth]{cov_me_lin.pdf}
	\label{fig:cov_lin:me}
}

\subfloat[HME-L-4]{
	\includegraphics[width=0.6\linewidth]{cov_hme_lin.pdf}
	\label{fig:cov_lin:hme}
}
\end{center}
\caption{Covariance matrix of gating values of generators for ME-L-16 and HME-L-4.}
\vskip\baselineskip
...
\begin{center}
\subfloat[ME-L-16]{
	\includegraphics[width=0.75\linewidth]{me_mnist_L4_exps.png}
	\label{fig:gens:mnist:me}
}

\subfloat[HME-L-4]{
	\includegraphics[width=0.75\linewidth]{hme_mnist_L4_exps.png}
	\label{fig:gens:mnist:hme}
}

\subfloat[ME-L-16]{
	\includegraphics[width=0.75\linewidth]{me_celeb_L4_exps.png}
	\label{fig:gens:celeb:me}
}

\subfloat[HME-L-4]{
	\includegraphics[width=0.75\linewidth]{hme_celeb_L4_exps.png}
	\label{fig:gens:celeb:hme}
}
\end{center}
\caption{Some samples generated from generators at the leaves of ME-L-16 and HME-L-4. Each column represents samples from different generators. Each generator produces samples in a different input region. To find appropriate input regions of generators, we sampled 10,000 input noise vectors and select the top five samples gating values of generators are maximized.}
\vskip\baselineskip
\label{fig:gens}
\end{figure}
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores on MNIST data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores on FashionMNIST data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores on CelebA data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores on UTZap50K data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\begin{table}[thbp]
\vskip\baselineskip
\caption{5-NN accuracies and FID scores on Oxford Flowers data set.}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
Model & 5-NN Real & 5-NN Fake & FID & Param. \\
...
\subfloat[FashionMNIST]{
	...
	\label{fig:box:fashion}
}
\end{center}
\caption{FID scores and 5-NN accuracies on MNIST and FashionMNIST data sets. The parameter count does not include the convolutional part.}
\vskip\baselineskip
\label{fig:box1}
\end{figure}
...
\caption{The average node responses of HME-L-4 model on FashionMNIST.}
\vskip\baselineskip
...
\caption{The average node responses of HME-L-5 model on CelebA.}
\vskip\baselineskip
...
\caption{The average node responses of HME-L-4 model on UTZap50K}
\vskip\baselineskip
...
\caption{The average node responses of HME-L-5 model on Oxford Flowers. The color variation increases in the nodes as we go down in the tree.}
\vskip\baselineskip
