...
\label{sec:flat_gen}
Our first proposed architecture uses mixtures of experts \cite{me}
...
Wasserstein loss \cite{arjovsky2017wasserstein} with gradient penalty \cite{improved_wgan} is used. We adopted the suggested hyperparameter setting of Wasserstein loss recommended in \cite{improved_wgan}, namely two-sided gradient penalty with a constant of $10.0$. The discriminator is trained 5 times per optimization step of the generator. We used Adam optimizer \cite{adam} with amsgrad option \cite{amsgrad}. Learning rate is set to $0.0001$ with beta values of Adam set to $(0.5, 0.999)$. We do not apply learning rate decay. Batch size is set to 128.
